...
setwd("~/Dropbox/PhD Folder/Virtual Lab Book/Leuven_Samples/Transcription/Diff_exp_files")
...
DE_Genes <- read.table("LOC_head_upreg_nonrepro.txt",header = F)
colnames(DE_Genes) <- "genes"

# change class to a vector
DE_Genes <- as.vector(DE_Genes[,1])

# Keeping only DE genes that been annotated

DE_Genes_A <- DE_Genes[DE_Genes %in% universe]
...
write.table(REVIGO,"enriched_GO_head_upreg_nonrepro.txt",row.names = F,sep = "\t",quote = F)
