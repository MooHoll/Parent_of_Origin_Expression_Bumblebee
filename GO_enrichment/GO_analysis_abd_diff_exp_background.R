...
GO_annotations <- read.table("GOs_all_adb_diffexp_genes_as_background.txt", header = T)
...
#my_genes <- read.csv("./gene_lists/LOC_abd_upreg_nonrepro.txt",header=F)
my_genes <- read.csv("./gene_lists/LOC_abd_upreg_repro.txt",header=F)
...
#3723/4365 Upreg sterile
#3201/3732 Upreg repro
...
#write.table(REVIGO,"enriched_GOs_upreg_sterile_abd_genes.txt",row.names = F,sep = "\t",quote = F)
write.table(REVIGO,"enriched_GOs_upreg_repro_abd_genes.txt",row.names = F,sep = "\t",quote = F)
