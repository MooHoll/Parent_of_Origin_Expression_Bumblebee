...
GO_annotations <- read.table("GOs_all_head_diffexp_genes_as_background.txt", header = T)
...
#my_genes <- read.csv("./gene_lists/LOC_head_upreg_nonrepro.txt",header=F)
my_genes <- read.csv("./gene_lists/LOC_head_upreg_repro.txt",header=F)
...
#write.table(REVIGO,"enriched_GOs_upreg_sterile_head_genes.txt",row.names = F,sep = "\t",quote = F)
write.table(REVIGO,"enriched_GOs_upreg_repro_head_genes.txt",row.names = F,sep = "\t",quote = F)
